...
<template>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id" :class="{ highlight: isHighlighted(author) }">
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
      <button @click="toggleHighlight(1)">Toggle Highlight Jane Austen</button>
      <button @click="toggleHighlight(2)">Toggle Highlight George Orwell</button>
      <button @click="toggleHighlight(3)">Toggle Highlight Agatha Christie</button>
    </section>
    </div>
  </template>
  ...
  const orwell = computed((name) => {
    // TODO: CODE TO FIND AUTHOR BY NAME HERE
    return authors.find((author) => author.name === 'George Orwell')
  })
  
  // Activity 5: Find author by ID
  const austen = computed((ID) => {
    // TODO: CODE TO FIND AUTHOR BY ID HERE
    return authors.find((author) => author.ID === 200 )
  })
  


const toggleHighlight = (id) => {
  if (highlightedAuthors.value.includes(id)) {
    highlightedAuthors.value = highlightedAuthors.value.filter(authorId => authorId !== id)
  } else {
    highlightedAuthors.value.push(id)
  }
}

const isHighlighted = (author) => {
  return highlightedAuthors.value.includes(author.id)
}
</script>
